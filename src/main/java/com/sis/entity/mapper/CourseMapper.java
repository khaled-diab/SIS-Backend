...
public class CourseMapper implements Mapper<Course, CourseDTO> {
	...
	@Override
	public ArrayList<CourseDTO> toDTOs(Collection<Course> entity) {
		return entity.stream().map(this::toDTO).collect(toCollection(ArrayList<CourseDTO>::new));
	}

	@Override
	public PageResult<CourseDTO> toDataPage(PageResult<Course> entity) {
		return new PageResult<>(entity.getData().stream().map(this::toDTO).collect(toCollection(ArrayList<CourseDTO>::new)), entity.getTotalCount(), entity.getPageSize(), entity.getCurrPage());
	}
