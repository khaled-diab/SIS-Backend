...
public class StudentMapper implements Mapper<Student, StudentDTO> {
    ...
    public ArrayList<StudentDTO> toDTOs(Collection<Student> students) {
        return students.stream().map(this::toDTO).collect(toCollection(ArrayList<StudentDTO>::new));
    }

    @Override
    public ArrayList<Student> toEntities(Collection<StudentDTO> studentDTOS) {
        return studentDTOS.stream().map(this::toEntity).collect(toCollection(ArrayList<Student>::new));
    }

    @Override
    public PageResult<StudentDTO> toDataPage(PageResult<Student> entities) {
        return new PageResult<>(entities.getData()
                .stream()
                .map(this::toDTO)
                .collect(toCollection(ArrayList<StudentDTO>::new)), entities.getTotalCount(), entities.getPageSize(), entities.getCurrPage());
